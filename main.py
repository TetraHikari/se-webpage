...
async def courses(request: Request, username: str = Cookie(None), is_professor: bool = Cookie(None), email: str = Cookie(None), firstname: str = Cookie(None), lastname: str = Cookie(None), subject: str = Cookie(None)):
    root = open_db_client()
    try:
        return templates.TemplateResponse("courses.html", {"request": request, "username": username, "is_professor": is_professor, "email": email, "name": firstname+" "+lastname, "subject": subject})
    finally:
        shutdown_db_client()
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="localhost", port=14000)